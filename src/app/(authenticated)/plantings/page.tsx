...
export default async function PlantingsPage({ searchParams }: PageProps) {
  ...
  if (!session?.user?.id) {
    return <div>Please sign in to view your plantings.</div>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Planting Log</h1>
      ...
      {plantings.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No plantings found</h3>
                  ...
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      <Link 
                        ...
                        className="font-semibold text-gray-900 dark:text-white hover:text-garden-600 dark:hover:text-garden-400"
                      >
                      ...
                      <div className="mt-3 space-y-2">
                        ...
                        <div className="pl-6 space-y-1">
                          {planting.plantingEvents.slice(0, 3).map((event) => (
                            <div key={event.id} className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-2">
                      ...
                      {planting.harvestDate && (
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                          <span className="text-green-600 dark:text-green-400 font-medium">Harvested:</span> {formatDate(planting.harvestDate)}
                          {planting.actualYield && ` — ${planting.actualYield}`}
                        </p>
                      )}
                      
                      {planting.notes && (
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 italic">{planting.notes}</p>
                      )}
                    </div>
                  </div>
