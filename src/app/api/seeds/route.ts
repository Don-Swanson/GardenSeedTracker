...
export async function POST(request: Request) {
  try {
    ...
    // Support both customPlantName and legacy 'name' field for backward compatibility
    const customPlantName = sanitizeText(data.customPlantName || data.name, MAX_LENGTHS.name)
    // Support both customCategory and legacy 'category' field for backward compatibility  
    const customCategory = sanitizeText(data.customCategory || data.category, MAX_LENGTHS.name)
    ...
    // Only store custom plant info if no plant type from encyclopedia is selected
    const finalCustomPlantName = plantTypeId ? null : customPlantName
    const finalCustomCategory = plantTypeId ? null : customCategory
    ...
    const seed = await prisma.seed.create({
      data: {
        ...
        plantingDepth,
        plantTypeId,
        customPlantName: finalCustomPlantName,
