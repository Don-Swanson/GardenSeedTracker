    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-900">Planting Details</h2>
          {!editingDetails ? (
            <button
        ...
        {!editingDetails ? (
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="ml-2 font-medium">{planting.locationName}</span>
            </div>
            <div>
              <span className="text-gray-500">Total Quantity:</span>
              <span className="ml-2 font-medium">{planting.quantityPlanted}</span>
            </div>
            <div>
              <span className="text-gray-500">Status:</span>
              <span className="ml-2 font-medium">{statusInfo?.label || planting.status}</span>
            </div>
            {planting.harvestDate && (
              <div>
                <span className="text-gray-500">Harvest Date:</span>
                <span className="ml-2 font-medium">{formatDate(planting.harvestDate)}</span>
              </div>
            )}
            {planting.actualYield && (
              <div className="col-span-2">
                <span className="text-gray-500">Yield:</span>
                <span className="ml-2 font-medium">{planting.actualYield}</span>
              </div>
            )}
            {planting.notes && (
              <div className="col-span-2">
                <span className="text-gray-500">Notes:</span>
                <p className="mt-1 text-gray-700">{planting.notes}</p>
              </div>
            )}
          </div>
        ) : (
        ...
        {planting.plantingEvents.length === 0 ? (
          ...
          <div className="space-y-3">
            {planting.plantingEvents.map((event) => (
              <div 
                key={event.id} 
                className="border border-gray-200 rounded-lg p-4 hover:border-garden-300 transition-colors"
              >
                {editingEventId === event.id ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 bg-garden-100 rounded-full flex items-center justify-center flex-shrink-0">
