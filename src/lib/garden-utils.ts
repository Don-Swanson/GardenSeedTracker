...
export function getMoonPhase(date: Date): {
  ...
  // Approximation based on synodic month (29.53 days)
  let c = 0
  let e = 0
  let jd = 0
  let b = 0
  ...
  const phases: Record<number, { phase: string; emoji: string; plantingAdvice: string }> = {
    0: { phase: 'New Moon', emoji: '🌑', plantingAdvice: 'Best for planting leafy crops that produce seeds outside the fruit' },
    1: { phase: 'Waxing Crescent', emoji: '🌒', plantingAdvice: 'Good for leafy annuals with external seeds' },
    2: { phase: 'First Quarter', emoji: '🌓', plantingAdvice: 'Best for fruiting annuals with internal seeds (tomatoes, peppers)' },
    3: { phase: 'Waxing Gibbous', emoji: '🌔', plantingAdvice: 'Good for transplanting and grafting' },
    4: { phase: 'Full Moon', emoji: '🌕', plantingAdvice: 'Best for planting root crops and perennials' },
    5: { phase: 'Waning Gibbous', emoji: '🌖', plantingAdvice: 'Good for root vegetables and bulbs' },
    6: { phase: 'Last Quarter', emoji: '🌗', plantingAdvice: 'Best for weeding, harvesting, and pest control' },
    7: { phase: 'Waning Crescent', emoji: '🌘', plantingAdvice: 'Rest period - avoid planting. Good for soil work' },
  }
  
  const phaseData = phases[phase % 8]
  
  return {
    ...phaseData,
    illumination: phase <= 4 ? (phase / 4) * 100 : ((8 - phase) / 4) * 100,
  }
}
...
export const plantingStatuses = [
  ...
  { value: 'growing', label: 'Growing', color: 'bg-emerald-100 text-emerald-800' },
  { value: 'harvested', label: 'Harvested', color: 'bg-blue-100 text-blue-800' },
